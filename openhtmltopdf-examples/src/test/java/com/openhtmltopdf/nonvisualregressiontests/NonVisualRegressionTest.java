...
public class NonVisualRegressionTest {
	...
	private static class RectangleCompare extends CustomTypeSafeMatcher<PDRectangle> {
		...
		protected boolean matchesSafely(PDRectangle item) {
			assertEquals(cssPixelsToPdfPoints(expec.getLowerLeftX()), item.getLowerLeftX(), 1.0d);
			assertEquals(cssPixelsToPdfPoints(expec.getUpperRightX()), item.getUpperRightX(), 1.0d);

			// Note: We swap the Ys here because PDFBOX returns a rect in bottom up units while expected is in topdown units.
			assertEquals(cssPixelYToPdfPoints(expec.getUpperRightY(), pageHeight), item.getLowerLeftY(), 1.0d);
			assertEquals(cssPixelYToPdfPoints(expec.getLowerLeftY(), pageHeight), item.getUpperRightY(), 1.0d);
	...
	/**
	 * Check that an html containing corrupt (incomplete base64) images can still be converted to PDF
	 */
	@Test
	public void testIgnoreCorruptedImages() throws IOException {
		String htmlFileName = "ignore-corrupt-image";
		PDDocument doc = run(htmlFileName);
		assertEquals("No image object expected.", false, doc.getPage(0).getResources().getXObjectNames().iterator().hasNext());
		remove(htmlFileName, doc);
	}
